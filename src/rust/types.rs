...
pub fn write_structs<W: Write>(writer: &mut Writer<W>, structs: Structs, public: bool, symbiosis_path: &str) -> Result<(), Error> {
    for s in structs {
        if public {
...

        {
            ...
            for (parameter, ty) in &s.fields {
                ...
                match write_ty(&mut line, ty, symbiosis_path) {
...
fn write_ty<W: Write>(w: &mut Line<W>, ty: &Type, symbiosis_path: &str) -> Result<(), Error> {
    if ty.is_optional() {
        try_w!(w, "Option<");
    }

    match ty {
        &Type::Content(_) => try_w!(w, "{}::Content<'a>", symbiosis_path),
        &Type::Bool => try_w!(w, "bool"),
        &Type::Collection(Some(ref inner), _) => {
            try_w!(w, "{}::Collection<'a, ", symbiosis_path);
            try!(write_ty(w, inner, symbiosis_path));
            try_w!(w, ">");
        },
